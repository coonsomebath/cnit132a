const gallery=Vue.createApp({template:`
    ...
    `,data(){return{items:{}}},async beforeMount(){try{const resp=await fetch('src/static/data/gallery/gallery.min.json');if(!resp.ok){throw new Error('Failed to fetch json data.')}
this.items=await resp.json()}catch(error){console.error("Fetching gallery.json failed:",error)}}})
